"use client";
type Props = { title: string; message?: string; confidence?: number; tone?: 'success'|'warn'|'error'|'info' };
  ...
  return (
    <div className={`rounded-lg border p-4 ${toneClasses[tone]}`}>
      ...
      {typeof confidence === 'number' && (
        <div className="text-xs text-zinc-500 mt-2">Confidence: {Math.round(confidence*100)}%</div>
      )}
    </div>
  );
}

...
export default function CitySwitch() {
  ...
  return (
    <div ref={containerRef} className="relative text-sm">
      ...
      {open && (
        <ul role="listbox" className="absolute right-0 mt-1 w-40 rounded-lg border bg-white shadow-md dark:bg-zinc-900 dark:border-zinc-700 z-50">
          {CITIES.map(c => (
            <li key={c.code} role="option" aria-selected={preferredCity===c.code}>
              <button
                onClick={() => { setPreferredCity(c.code); setOpen(false); }}

...
export async function POST(req: Request) {
  try {
    ...
    const title =
      locale === "de" ? "Antrag auf Umgangsregelung" : "Contact/Visitation order application";
    doc.setTitle(title);
    doc.setSubject(
      locale === "de"
        ? "Amtsgericht – Familiengericht; Vorgeschlagener Umgang; Einstweilige Anordnung; Unterschrift"
        : "Local Court – Family Court; Proposed schedule; Interim relief; Signature"
    );
    doc.setKeywords([
      title,
    ...
    const roleLabel = (r?: PartyRole) =>
      r === "parentA" ? "Elternteil A" : r === "parentB" ? "Elternteil B" : "";
    ...
    if (proposal && proposal.weekday) {
      ...
      drawField(
        locale === "de" ? "Wochenende (gerade)" : "Weekend even",
        String(weekend.even || "-")
      );
      drawField(
        locale === "de" ? "Wochenende (ungerade)" : "Weekend odd",
        String(weekend.odd || "-")
      );
      const handover = proposal.handover || {};
      if (handover.location)
        drawField(locale === "de" ? "Übergabe" : "Handover", handover.location);
    }
    ...
    if (formData.interim) {
      ...
      drawField(
        "",
        locale === "de"
          ? "Es wird hilfsweise die einstweilige Anordnung beantragt."
    ...
    // Optional: add timeline excerpt as a final page
    if (timelineText && timelineText.trim().length > 0) {
      ...
      const drawLine = (text: string) => {
        ...
        page.drawText(text, { x: MARGIN_X, y: y2, size: 10, font });
        y2 -= 12;
      };
      const heading = locale === "de" ? "Zeitleiste (Auszug)" : "Timeline (excerpt)";
      page.drawText(heading, { x: MARGIN_X, y: 800, size: 12, font });
      const maxWidth = PAGE_SIZE[0] - MARGIN_X * 2;
      const lines = wrapText(timelineText.slice(0, 4000), maxWidth, font, 10);

...
export async function POST(req: Request) {
  try {
    ...
    const base = process.env.OPENAI_API_BASE || process.env.AI_API_BASE || 'https://api.openai.com/v1';
    ...
    if (!apiKey) {
      const suggestion: ClarifyResponse = {
        suggestion: 'unsure',
        confidence: 0.35,
        followup: body.questionText ? `Können Sie genauer sagen: ${body.questionText}?` : 'Bitte prüfen Sie die Hilfe und Quellen.',
    ...
    if (!res.ok) {
      ...
      return new Response(JSON.stringify({ error: 'AI request failed', detail: text }), { status: 502 });
    }
    ...
    if (!parsed || typeof parsed.suggestion !== 'string' || typeof parsed.confidence !== 'number') {
      const fallback: ClarifyResponse = { suggestion: 'unsure', confidence: 0.5, followup: 'Bitte prüfen Sie die Hilfe.', reasoning: 'Unparsable model output.' };
